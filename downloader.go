...
type Downloader interface {
	// Returns page reader
	Page(ctx context.Context, mediaID, n int, format string) (io.ReadCloser, error)
	/// Returns page thumbnail reader
	Thumbnail(ctx context.Context, mediaID int, n int, format string) (io.ReadCloser, error)
	// Returns cover reader
	Cover(ctx context.Context, mediaID int, format string) (io.ReadCloser, error)
}
...
func downloadOne(ctx context.Context, downloader Downloader, d Doujinshi, i int, name string) error {
	file, err := os.OpenFile(name, os.O_RDWR|os.O_CREATE, 0o755)
	if err != nil {
		return err
	}
	defer file.Close()
	...
	r, err := downloader.Page(ctx, d.MediaID, i+1, format)
	if err != nil {
		return err
	}
	defer r.Close()
...
func DownloadAll(ctx context.Context, downloader Downloader, d Doujinshi, namer func(i int, d Doujinshi) string) error {
	for i := 0; i < d.NumPages; i++ {
		name := namer(i+1, d)
		if err := downloadOne(ctx, downloader, d, i+1, name); err != nil {
			return err
		}
	}
	return nil
